...
class PiecesArgparser(argparse.ArgumentParser): # subclassing the ArgumentParser class to modify the error messages
    ...
    def error(self, message):
        if 'invalid choice' in message:
            try:
                ...
                suggestion_text = f"Did you mean '{self.find_most_similar_command(list(self._subparsers._group_actions[0].choices.keys()),invalid_command)}'?"
    ...
    def find_most_similar_command(cls,valid_commands, user_input):
        ...
        distances = {cmd: cls.levenshtein_distance(user_input, cmd) for cmd in valid_commands}
