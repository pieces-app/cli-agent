...
class EventHub:
    ...
    def _post_to_current_screen(self, message):
        """Post message to current screen to ensure proper routing."""
        try:
            # Get the current screen from the app
            current_screen = self.app.screen
            if current_screen:
                current_screen.post_message(message)
            else:
                # Fallback to app if no current screen
                self.app.post_message(message)
        except Exception as e:
            Settings.logger.error(f"Error posting message to current screen: {e}")
            # Fallback to app
            self.app.post_message(message)
    ...
    def _setup_event_bridging(self):
        ...
        self.copilot.on(
            ...
            lambda _: self._post_to_current_screen(CopilotMessages.ThinkingStarted()),
        )
        self.copilot.on(
            EventType.COPILOT_THINKING_ENDED,
            lambda _: self._post_to_current_screen(CopilotMessages.ThinkingEnded()),
        )
        self.copilot.on(
            EventType.COPILOT_STREAM_STARTED,
            lambda data: self._post_to_current_screen(CopilotMessages.StreamStarted(data)),
        )
        self.copilot.on(
            EventType.COPILOT_STREAM_CHUNK,
            lambda data: self._post_to_current_screen(
                CopilotMessages.StreamChunk(data["text"], data["full_text"])
            ),
        )
        self.copilot.on(
            EventType.COPILOT_STREAM_COMPLETED,
            lambda data: self._post_to_current_screen(CopilotMessages.StreamCompleted(data)),
        )
        self.copilot.on(
        ...
        self.workstream.on(
            ...
            lambda summary: self._post_to_current_screen(WorkstreamMessages.Updated(summary)),
        )
        self.workstream.on(
            EventType.WORKSTREAM_SUMMARY_DELETED,
            lambda summary_id: self._post_to_current_screen(WorkstreamMessages.Deleted(summary_id)),
        )
        self.workstream.on(
            EventType.WORKSTREAM_SUMMARY_SWITCHED,
            lambda summary: self._post_to_current_screen(WorkstreamMessages.Switched(summary)),
        )
