...
from pieces._vendor.pieces_os_client.wrapper.version_compatibility import UpdateEnum, VersionCheckResult
...
class TestMCPGatewayValidation:
    ...
    async def test_validate_system_status_pieces_os_not_running(self, mock_connection):
        ...
        with patch.object(mock_connection, '_check_pieces_os_status', return_value=False):
            is_valid, error_message = mock_connection._validate_system_status("ask_pieces_ltm")
    ...
    @pytest.mark.asyncio  
    async def test_validate_system_status_version_incompatible_plugin_update(self, mock_connection):
        """Test validation when CLI version needs updating"""
        # Mock PiecesOS running
        with patch.object(mock_connection, '_check_pieces_os_status', return_value=True):
    ...
    @pytest.mark.asyncio
    async def test_validate_system_status_version_incompatible_pos_update(self, mock_connection):
    ...
    @pytest.mark.asyncio
    async def test_validate_system_status_ltm_disabled_create_memory_tool(self, mock_connection):
        ...
        with patch.object(mock_connection, '_check_pieces_os_status', return_value=True):
            ...
            # Mock LTM disabled
            with patch.object(mock_connection, '_check_ltm_status', return_value=False):
                is_valid, error_message = mock_connection._validate_system_status("create_pieces_memory")
    ...
    async def test_validate_system_status_non_ltm_tool_success(self, mock_connection):
        ...
        with patch.object(mock_connection, '_check_pieces_os_status', return_value=True):
            ...
            with patch.object(mock_connection, '_check_ltm_status', return_value=False):
                is_valid, error_message = mock_connection._validate_system_status("some_other_tool")
    ...
    async def test_call_tool_with_validation_failure(self, mock_connection):
        ...
        with patch.object(mock_connection, '_validate_system_status', return_value=(False, error_msg)):
    ...
    async def test_call_tool_with_connection_failure(self, mock_connection):
        ...
        with patch.object(mock_connection, '_validate_system_status', return_value=(True, "")):
            # Mock connection failure
            with patch.object(mock_connection, 'connect', side_effect=Exception("Connection failed")):
                result = await mock_connection.call_tool("test_tool", {})
    ...
    def test_check_pieces_os_status_health_ws_running(self, mock_health_ws, mock_connection):
    ...
    def test_check_pieces_os_status_starts_health_ws(self, mock_settings, mock_health_ws, mock_connection):
    ...
    async def test_try_get_upstream_url_success(self, mock_connection):
        ...
        with patch('pieces.mcp.gateway.Settings') as mock_settings, \
             patch('pieces.mcp.gateway.get_mcp_latest_url', return_value="http://test-url"):
