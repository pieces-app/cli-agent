...
def update_application(conn, app):
    """ Update an existing application in the applications table """
    try:
        cursor = conn.cursor()
        cursor.execute("UPDATE applications SET name=:name, version=:version, platform=:platform, onboarded=:onboarded, privacy=:privacy WHERE id=:id",
