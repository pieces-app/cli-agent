...
prod_tag=$(echo "$prod_json" | jq -r '.tag')
...
staging_tag=$(echo "$staging_json" | jq -r '.tag')
...
prod_major=$(echo "$prod_tag" | cut -d. -f1)
prod_minor=$(echo "$prod_tag" | cut -d. -f2)
prod_patch=$(echo "$prod_tag" | cut -d. -f3)

staging_major=$(echo "$staging_tag" | cut -d. -f1)
staging_minor=$(echo "$staging_tag" | cut -d. -f2)
staging_patch=$(echo "$staging_tag" | cut -d. -f3)
staging_dev=$(echo "$staging_tag" | grep -oP '(?<=\.dev)\d+' || echo 0)
...
# If prod hasn't changed, increment the dev number
    ...
    if [ "$staging_dev" -eq 0 ]; then
        final_version="$staging_major.$staging_minor.$staging_patch.dev1"
    else
